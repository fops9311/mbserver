...
func NewRTUFrame(packet []byte) (*RTUFrame, error) {
	...
	// Calculate checksum
	var crc crc
	crc.reset().pushBytes(packet[0 : pLen-2])
	checksum := uint16(packet[pLen-1])<<8 | uint16(packet[pLen-2])
	if checksum != crc.value() {
		return nil, fmt.Errorf("RTU Frame error: CRC (expected 0x%x, got 0x%x)", crc.value(), checksum)
	}
